package net.rebel459.drops_backport.worldgen.feature.poplar;
...
import net.rebel459.drops_backport.registry.DBBlocks;
...
public class ShelfMushroomDecorator extends TreeDecorator {
    ...
    protected TreeDecoratorType<?> type() {
        return DBWorldgenCodecs.SHELF_MUSHROOM_DECORATOR.get();
    }
    ...
    private static boolean hasShelfMushroomAt(final TreeDecorator.Context context, final BlockPos pos) {
        return context.checkBlock(pos, state -> state.is(DBBlocks.SHELF_MUSHROOM));
    }
    ...
    public static boolean isBlockReplaceableWithShelfMushroom(final TreeDecorator.Context context, final BlockPos pos) {
        return context.checkBlock(pos, BlockBehaviour.BlockStateBase::canBeReplaced) && !isWaterOrWaterNearby(context, pos);
    }

    public static boolean isWaterOrWaterNearby(final TreeDecorator.Context context, final BlockPos pos) {
        return context.checkBlock(pos, state -> state.is(Blocks.WATER))
                || context.checkBlock(pos.east(), state -> state.is(Blocks.WATER))
                || context.checkBlock(pos.west(), state -> state.is(Blocks.WATER))
                || context.checkBlock(pos.north(), state -> state.is(Blocks.WATER))
                || context.checkBlock(pos.south(), state -> state.is(Blocks.WATER));
    }
}
